    ...
    public static class TelerikExtensions
    {
        ...
        public static SelectList ToSelectList<TEnum>(this TEnum enumObj, bool markCurrentAsSelected = true) where TEnum : struct
        {
            ...
            var values = from TEnum enumValue in Enum.GetValues(typeof(TEnum))
                         select new { ID = Convert.ToInt32(enumValue), Name = enumValue };// enumValue.GetLocalizedEnum(localizationService, workContext) };
            ...
            bool convertToUserTime, string defaultFormat)
        {
            ...
            else
            {
                ...
                {
                    tmp1 = DateTime.SpecifyKind(tmp1, DateTimeKind.Utc);
                    tmp1 = TimeZoneInfo.ConvertTimeToUtc(tmp1);
                }
                //default formatting
        ...
        public static GridFilter GetCommandFilter(this GridCommand command)
        {
            IDictionary<string, object> parameters = new Dictionary<string, object>();
            string sortcommand = null;
            if (command.SortDescriptors != null && command.SortDescriptors.Count > 0)
            {
                string sortDirection = ListSortDirection.Ascending == command.SortDescriptors[0].SortDirection ? "asc" : "desc";
                sortcommand = string.Format("order by {0} {1}", command.SortDescriptors[0].Member, sortDirection);
            }

...
namespace AgileEAP.Core.Caching
{
    public class CacheManager
    {
        ...
        private static ICache cache = null;// CacheFactory.CreateCache<MemoryCache>();
        ...
        static CacheManager()
        {
            try
            {
                if (cache == null)
                {
                    string cacheName = Configure.DirectGet<string>("CacheName", "MemoryCache");
                    cache = CacheFactory.CreateCache(cacheName);
                }
            }
            catch (Exception ex)
            {
                GlobalLogger.Error(string.Format("初始化CacheManager出错{0}", ex.Message));
                cache = CacheFactory.CreateCache<MemoryCache>();
            }
        }

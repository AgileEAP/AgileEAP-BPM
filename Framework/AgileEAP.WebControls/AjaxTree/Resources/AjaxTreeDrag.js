...
var Drag = function(id) {
    ...
    var dragstart = function(e) {
        ...
        dragNode = document.getElementById("ajaxTreeDragContainer");
        dragNode.style.position = 'absolute';
        dragNode.style.display = 'none';
        activeCurrentNode($(el).attr("id"));

        var dragIcon = document.createElement("img");
        dragIcon.setAttribute("src", ajaxTreeSetting.ajaxTree_drag_enable);
    ...
    var drag = function(e) {
        ...
        if (target.parentNode != el && target.parentNode.tagName == "LI" && target.tagName == "A" && target.getAttribute("dragNode")) {
            ...
            if ($(el).find("#" + id)[0]) {
                $(dragNode).children("img:first").attr("src", ajaxTreeSetting.ajaxTree_drag_disable);
            }
            else {
                $(dragNode).children("img:first").attr("src", ajaxTreeSetting.ajaxTree_drag_enable);
            }
        }
        else {
            $(dragNode).children("img:first").attr("src", ajaxTreeSetting.ajaxTree_drag_disable);
    ...
    var dragend = function(e) {
        ...
        if (target.parentNode != el && target.parentNode.tagName == "LI" && target.tagName == "A" && target.getAttribute("dragNode")) {
            ...
            if (ul[0]) {
                ...
                nodeIcon.attr("src", ajaxTreeSetting.ajaxTree_minus);
        ...
        if (parentNode[0]) {
            ...
            if (prevNode[0]) {
                ...
                if (ul[0]) {
                    ...
                    prevNode.find("img:first").attr("src", ajaxTreeSetting.ajaxTree_bottom_line);
                }
                prevNode.attr("style", "background:url(" + ajaxTreeSetting.ajaxTree_bottom_line + ")  white no-repeat;_background: url(" + ajaxTreeSetting.ajaxTree_bottom_line + ") white no-repeat 0px 3px;");
                ...
                nodeIcon.attr("src", parentNode.next("li").size() > 0 ? ajaxTreeSetting.ajaxTree_middle_line : ajaxTreeSetting.ajaxTree_bottom_line);
                nodeIcon.unbind("click");
    ...
    var refreshNodeState = function(node) {
        if (node[0]) {
            ...
            if (!ul[0]) {
                var liIcon = node.next("li").size() > 0 ? ajaxTreeSetting.ajaxTree_middle_line : ajaxTreeSetting.ajaxTree_bottom_line;
            ...
            if (node.next("li").size() > 0) {
                ul.attr("style", "background:url(" + ajaxTreeSetting.ajaxTree_vertical_line + ")  white repeat-y;_background: url(" + ajaxTreeSetting.ajaxTree_vertical_line + ") white repeat-y 0px;");
            }
            else {
                ul.removeAttr("style");
            }
                ...
                ul.children("li").each(function() {
                    ...
                    if (childrenCount == 1) {
                        if (li.children("ul").size() > 0)
                            li.attr("style", "background:url(" + ajaxTreeSetting.ajaxTree_bottom_line + ")  white no-repeat;_background: url(" + ajaxTreeSetting.ajaxTree_bottom_line + ") white no-repeat 0px;");
                        else {
                            var nodeIcon = li.children("img:first");
                            nodeIcon.attr("src", ajaxTreeSetting.ajaxTree_bottom_line);
                            nodeIcon.unbind("click");
                            li.removeAttr("style");
                        }
                    }
                    else {
                        var liIcon = li.next("li").size() > 0 ? ajaxTreeSetting.ajaxTree_middle_line : ajaxTreeSetting.ajaxTree_bottom_line;

            ...
            foreach (var constructor in constructors)
            {
                try
                {
                    ...
                    foreach (var parameter in parameters)
                    {
                        ...
                        if (service == null) throw new AgileEAP.Core.ExceptionHandler.EAPException("Unkown dependency");
                        parameterInstances.Add(service);
                    }
                    return Activator.CreateInstance(type, parameterInstances.ToArray());
                }
                catch (AgileEAP.Core.ExceptionHandler.EAPException ex)
                {
                    GlobalLogger.Error(ex);
                }
            }
            throw new AgileEAP.Core.ExceptionHandler.EAPException("No contructor was found that had all the dependencies satisfied.");
        }

 ...
 */
window.defaultsElement =
    {
        selected: 0,
        edited: 0,
        // Selected Step, 0 = first step   
        keyNavigation: true,
        // Enable/Disable key navigation(left and right keys are used if enabled)
        enableAllSteps: false,
        transitionEffect: '',
        // Effect on navigation, none/fade/slide/slideleft
        contentURL: null,
        // content url, Enables Ajax content loading
        contentCache: true,
        // cache step contents, if false content is fetched always from ajax url
        cycleSteps: false,
        // cycle step navigation
        enableFinishButton: false,
        // make finish button enabled always
        errorSteps: [],
        // Array Steps with errors
        labelNext: '下一步',
        labelPrevious: '上一步',
        labelFinish: '完成',
        labelTitle: [],
...
(function ($) {
    $.fn.smartWizard = function (action) {
        if (action.edited == 1 || action.edited == true) {
            defaultsElement = action;
        }
        var options = $.extend({}, $.fn.smartWizard.defaults, action);
        options = $.extend({},options,defaultsElement);
        var args = arguments;
        ...
        return this.each(function () {
            ...
            for (var i = 0; i < steps.length; i++) {
                $(steps[i]).find("stepText").text(options.labelTitle[i]);
            }
            var contentWidth = 0;
            ...
            //if (options.formShow = 1 || options.formShow == true) {
            //    if ($.isFunction(options.onLeaveStep)) {
            //        obj.find("a").find(".selected")
            ...
            function init() {
                    ...
                    elmActionBar = $('<div></div>').addClass("actionBar");
                    elmStepContainer = $("<div id='StepContainer" + new Date().getTime() + "'></div>").addClass("stepContainer");
            ...
            function prepareSteps() {
                ...
                $(steps, obj).each(function (i) {
                    //  $($(this).attr("href"), obj).hide();qlj2012.12.11
                    $($(this).attr("href")).hide();
                    $(this).attr("rel", i + 1);
                });
            }
            ...
            function showStep(stepIdx) {
                ...
                //elmStepContainer.height($($(selStep, obj).attr("href"), obj).outerHeight());//qlj2012.12.10
                if (options.Tabs != 1) {
                    elmStepContainer.height(elmStepContainer.parent().height() - 78);
                }
                else
                    elmStepContainer.height(elmStepContainer.parent().height());
                elmStepContainer.find(".content").height(elmStepContainer.height());
                elmStepContainer.find(".content").width(elmStepContainer.width());
                if (options.transitionEffect == 'slide') {
                    ...
                    $($(curStep, obj).attr("href")).hide();//, obj).hide();
                    $($(selStep, obj).attr("href")).show();//, obj).show();
                    curStepIdx = stepIdx;
                    SetupStep(curStep, selStep);
                }
                return true;
            ...
            function SetupStep(curStep, selStep) {
                ...
                $(curStep, obj).parent().removeClass("selected");
                $(curStep, obj).parent().parent().removeClass("selected");
                $(selStep, obj).removeClass("disabled");
                $(selStep, obj).removeClass("done");
                $(selStep, obj).addClass("selected");
                $(selStep, obj).parent().addClass("selected");
                $(selStep, obj).parent().parent().addClass("selected");
                ...
                if ($.isFunction(options.onShowEditStep)) {
                    if (!options.onShowEditStep.call(this, $(selStep))) {
                        return false;
                    }
                }
            }
